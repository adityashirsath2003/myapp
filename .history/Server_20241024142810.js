...
app.post('/addreview', async (req, res) => {
    ...
    try {
        const sql = 'INSERT INTO reviews (mail, rate,title) VALUES (?, ?, ?)';
...
app.post('/addreview', async (req, res) => {
    ...
    try {
        const sql = 'delete from reviews where mail = ? and title = ?';
        const [results] = await pool.query(sql, [mail, title]);
...
app.get('/getallreviews', async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT id, mail, rate, title FROM reviews');
        res.status(200).json(rows);
    } catch (error) {
        console.error(error);
        res.status(500).json({ error: 'Error fetching books' });
